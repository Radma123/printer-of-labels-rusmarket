...
#   sx/sy — множители размера по горизонтали и вертикали, независимые:
#           можно ужать блок только по ширине или только по высоте.
#           sy влияет на поток (блоки ниже подвинутся сами), sx — нет.
#           Старый формат с одним общим `scale` читается как sx = sy = scale.
...
def default_tweaks() -> dict:
    return {key: {"dx": 0.0, "dy": 0.0, "sx": 1.0, "sy": 1.0} for key in ELEMENTS}
